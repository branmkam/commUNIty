      ...
      get(child(dbRef, '/')).then((snapshot) => {
          if (snapshot.exists()) {
            ...
            if(dbState.users[auth.currentUser.uid]) {
              let rests = dbState.users[auth.currentUser.uid].efaves.split(',').map(x => parseInt(x)).filter(x => !Number.isNaN(x))
              setFaves(rests.map(x => dbState.entertainment[x]))
            }
          }
      });
    }
